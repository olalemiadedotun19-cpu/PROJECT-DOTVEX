...
(async () => {
  const endpoints = [
    ['get_user_profile', { request: { hasUserName: true, userName: 'olalemiadedotun' } }],
    ['get_accelerator_quota', { request: {} }],
    ['get_notebook_info', { request: { userName: 'olalemiadedotun', kernelSlug: 'dotvex-qwen3-gpu-v2' } }],
    ...
    ['get_notebook_session_status', { request: { userName: 'olalemiadedotun', kernelSlug: 'dotvex-qwen3-gpu-v2' } }],
    ['list_notebook_session_output', { request: { userName: 'olalemiadedotun', kernelSlug: 'dotvex-qwen3-gpu-v2' } }],
  ];
  
  for (const [name, args] of endpoints) {
    const r = await call(name, args);
    const text = r && r.content ? r.content[0].text : JSON.stringify(r).substring(0, 200);
    const ok = !text.includes('Unauthenticated') && !text.includes('Permission');
    console.log(name + ':', ok ? 'OK' : 'FAIL - ' + text.substring(0, 120));
  }
})();
