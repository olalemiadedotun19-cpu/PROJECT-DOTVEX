...
(async () => {
  console.log('=== save_notebook (create + run) ===');
  const r = await call('save_notebook', {
    request: {
      newTitleNullable: 'DOTVEX Qwen3 GPU Test',
      slugNullable: 'olalemiadedotun/dotvexqwen3',
      textNullable: '# DOTVEX Qwen3 GPU test notebook\nprint("hello")\n',
  ...
  console.log('Result:', r && r.content ? r.content[0].text : JSON.stringify(r), 'isError=', r && r.isError);
})();
