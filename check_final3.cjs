...
(async () => {
  ...
  console.log('STATUS:', status && status.content ? status.content[0].text : JSON.stringify(status));
  
  const info = await call('get_notebook_info', { request: { userName: 'olalemiadedotun', kernelSlug: 'dotvex-qwen3-gpu-v2' } });
  const infoText = info && info.content ? info.content[0].text : JSON.stringify(info);
  const infoParsed = JSON.parse(infoText);
  console.log('VERSION:', infoParsed.metadata?.current_version_number);
  console.log('LAST_RUN:', infoParsed.metadata?.last_run_time);
  ...
  if (out && out.content) {
    ...
    console.log('OUTPUT_TAIL:', log.slice(-12000));
  }
})();
