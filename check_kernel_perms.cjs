...
(async () => {
  const r1 = await call('get_notebook_info', { request: { userName: 'olalemiadedotun', kernelSlug: 'dotvex-qwen3-gpu-v2' } });
  console.log('NOTEBOOK_INFO:', r1 && r1.content ? r1.content[0].text.substring(0, 800) : JSON.stringify(r1).substring(0, 800));
  
  const r2 = await call('list_notebook_files', { request: { userName: 'olalemiadedotun', kernelSlug: 'dotvex-qwen3-gpu-v2' } });
  console.log('NOTEBOOK_FILES:', r2 && r2.content ? r2.content[0].text.substring(0, 800) : JSON.stringify(r2).substring(0, 800));
})();
