...
(async () => {
  ...
  const st = await call('get_notebook_session_status', { request: { userName: 'olalemiadedotun', kernelSlug: 'dotvex-qwen3-gpu-test' } });
  console.log('STATUS:', st && st.content ? st.content[0].text : JSON.stringify(st));

  const out = await call('list_notebook_session_output', { request: { userName: 'olalemiadedotun', kernelSlug: 'dotvex-qwen3-gpu-test' } });
  console.log('\nOUTPUT LOG:');
  if (out && out.content) {
    const text = out.content[0].text;
    try {
      const parsed = JSON.parse(text);
      console.log(parsed.log || text);
    } catch (e) {
      console.log(text);
    }
  } else {
